  ...
  return (
    <div className='bg-[#f1f1f1] hover:bg-[#e0e0e0] p-6 text-4xl my-8' onClick={handleClickOnPost}>
      ...
      <div className='flex gap-20 items-center my-12 text-3xl'>
        <p>{post.client?.paymentVerified ?
          (<span><HiCheckCircle className='text-[#676767] inline-block' /> Payment verified</span>)
          :
          (<span><RxCrossCircled className='text-[#676767] inline-block' /> Payment unverified</span>)
        }
        </p>

        <Rating
          initialRating={post.client?.clientRating || 0}
          readonly
          emptySymbol={<span className="text-gray-400 text-5xl">☆</span>}
          fullSymbol={<span className="text-[#df7606] text-5xl">★</span>}
        />

        <p>${post.client?.clientSpent ?? 0} Spent</p>

        <div className='flex items-center'>
          <IoLocationOutline className='inline-block' /> {post.client?.clientLocation || "Unknown"}
        </div>
